...
static int gen_onoff_send(bool val)
{
	...
	int payload_size = (8+16+32+32+32+32)/8;
	...
	net_buf_simple_add_le32(&buf, extra_payload);
	net_buf_simple_add_le32(&buf, extra_payload);
	net_buf_simple_add_le32(&buf, extra_payload);
	net_buf_simple_add_le32(&buf, extra_payload);
...
static uint8_t cmp_uuid[2][16] = {	{0xe5, 0x54, 0x39, 0x6d, 0xf1, 0x11, 0xde, 0x36, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
									{0x95, 0xdf, 0xd0, 0x28, 0x02, 0x7e, 0x5a, 0x59, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}};

bool uuid_in_list(uint8_t uuid1[16], uint8_t uuid_list[2][16]){
	int i;
	int j;
	for(j = 0; j < 2; j++){
